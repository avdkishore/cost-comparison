(function ($) {
	...
	var fetchData = function (keyword) {
		...
		// fetch news content
		url = 'http://31.170.160.102/cgi-bin/amazon.py?keyword=' + keyword;
		imagesXhr = $.getJSON(url,
			function (response) {
				for (var count = response[0].length - 1; count >= 0; count--) {
					...
					details : 'details: ' + response[2][count]
				});
		...
		url = 'http://31.170.160.102/cgi-bin/taobao.py?keyword=' + keyword;
		imagesXhr = $.getJSON(url,
					function (response) {
						for (var count = response[0].length - 1; count >= 0; count--) {
							box_html = news_item_template({
		...
		url = 'http://31.170.160.102/cgi-bin/jd.py?keyword=' + keyword;
		imagesXhr = $.getJSON(url,
					function (response) {
						for (var count = response[0].length - 1; count >= 0; count--) {
							box_html = news_item_template({
